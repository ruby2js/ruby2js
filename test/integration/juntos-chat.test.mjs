...
//
// The demo contains:
// - app/models/message.rb - Message model with validations and broadcasting
// - app/controllers/messages_controller.rb - Controller actions
...
const DEMO_DIR = join(__dirname, 'workspace/chat');

describe('Chat Demo Integration Tests', () => {
  beforeAll(() => {
    // Verify the demo exists
    if (!existsSync(DEMO_DIR)) {
      throw new Error(`Demo not found at ${DEMO_DIR}. Run: node setup.mjs chat`);
    }

    // Verify test files exist
    if (!existsSync(join(DEMO_DIR, 'test/messages.test.mjs'))) {
