...
The self-hosted demo transpiles `lib/ruby2js/converter.rb` and related files to JavaScript using Ruby2JS itself. It passes the full transliteration test suite and supports the **functions filter** (`puts` → `console.log`, `.each` → `.forEach`, etc.).
