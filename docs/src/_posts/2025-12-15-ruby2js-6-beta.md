...
**npm package** — Install and run (no Ruby required):

```bash
npm install ruby2js@beta
npx ruby2js -e 'puts "hello"'
npx ruby2js --filter functions -e 'puts "hello"'  # → console.log("hello")
```
...
```html
<script type="module">
  import { convert } from 'https://www.ruby2js.com/demo/selfhost/ruby2js.js';
  console.log(convert('puts "hello"'));
</script>
```

Or visit the [self-hosted demo](https://www.ruby2js.com/demo/selfhost/) to try it interactively—including the **functions filter** with a checkbox toggle.
...
- 691 tests passing across 13 spec files
...
### Rails Pattern Transpilation

The new Rails filters enable a powerful workflow: write standard Rails code that transpiles to browser-ready JavaScript. Models, controllers, routes, and ERB templates all convert:
...
This enables offline-first applications where the same Ruby source runs on the server (with PostgreSQL) and in the browser (with IndexedDB). See the [Ruby2JS on Rails](https://intertwingly.net/blog/2025/12/21/Ruby2JS-on-Rails.html) post for a complete walkthrough—no Ruby installation required.
...
The beta includes full self-hosted versions of these filters:
...
| cjs | ✓ Ready | 23 |
| return | ✓ Ready | 25 |
| active_support | ✓ Ready | 16 |

**Rails Filters** (new in 6.0):

| Filter | Status | Tests |
|--------|--------|-------|
| rails/model | ✓ Ready | 21 |
| rails/controller | ✓ Ready | 19 |
| rails/schema | ✓ Ready | 25 |
| rails/seeds | ✓ Ready | 6 |
| rails/logger | ✓ Ready | 8 |

Before the final release:

- Completing partial specs: camelcase, polyfill, pragma, erb, comments
...
- **Retired filters**: matchAll (as announced in 2023)
- **Blocked filters** (awaiting selfhost support): underscore, vue, jquery, react, stimulus, jsx, lit, preact, nokogiri
...
*For the Rails transpilation demo, see [Ruby2JS on Rails](https://intertwingly.net/blog/2025/12/21/Ruby2JS-on-Rails.html).*
