...
Convert inline code:
```
ruby2js -e 'puts "Hello, World!"'
```

Use preset defaults with specific filters:
```
ruby2js --preset --filter camelCase myfile.rb
```
...
Start the interactive web demo (see [Running the Demo](running-the-demo)):
```
ruby2js --port 8080
```
