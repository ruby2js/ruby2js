...
class SelfhostBuilder
  ...
  def generate_unified_views_module(resource, views_by_name, views_dist_dir, source_formats)
    ...
    views_by_name.keys().sort.each do |view_name|
      ...
      # Import from compiled .js files for reliable bundling
      # The compiled files are generated by SelfhostBuilder alongside the source files
      compiled_filename = "#{view_name}.js"

      # Different file types have different export patterns
      case file_info[:ext]
      when '.html.erb'
        # ERB exports: export function render(...)
        unified_js += "import { render as #{view_name}_render } from './#{resource}/#{compiled_filename}';\n"
      ...
      when '.rb'
        ...
        unified_js += "import #{view_name}_module from './#{resource}/#{compiled_filename}';\n"
        render_exports << "#{view_name}: #{view_name}_module.render || #{view_name}_module"
      when '.rbx', '.jsx', '.tsx'
        # RBX/JSX exports: export default function/component
        unified_js += "import #{view_name}_component from './#{resource}/#{compiled_filename}';\n"
