module Ruby2JS
  class Converter
    ...
    # Created by pragma filter when # Pragma: logical or # Pragma: || is used
    # Forces || operator even when @or option would normally use ??

    handle :logical_or do |left, right|
    ...
    # Created by pragma filter when # Pragma: logical or # Pragma: || is used on ||=
    # Forces ||= operator even when @or option would normally use ??=

    handle :logical_asgn do |asgn, value|
      vtype = nil
