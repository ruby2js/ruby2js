    ...
    module Selfhost
      module Filter
        ...
        def on_send(node)
          ...
          # Handle Symbol === x → typeof x === "string"
          # In selfhost, Ruby symbols are represented as JavaScript strings
          if method_name == :=== && target&.type == :const &&
             target.children[0].nil? && target.children[1] == :Symbol
            # Symbol === x → typeof x === "string"
            return process s(:send,
              s(:send, nil, :typeof, args[0]),
              :===,
              s(:str, 'string')
            )
          end
