...
# Selfhost filter for transpiling the PrismWalker code
#
...
module Ruby2JS
  module Filter
    module SelfhostWalker
      include SEXP

      # Skip external requires that shouldn't be bundled
      SKIP_REQUIRES = %w[prism node].freeze

      # Handle negative index assignment: arr[-1] = x
      # Convert to: arr[arr.length - 1] = x
      def on_send(node)
        ...
        # Remove visibility declarations: private, protected, public
        # JavaScript doesn't have method-level visibility like Ruby
        if target.nil? && [:private, :protected, :public].include?(method)
          return s(:hide)
        end
