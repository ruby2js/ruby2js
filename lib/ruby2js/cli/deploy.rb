...
module Ruby2JS
  module CLI
    module Deploy
      ...
      TARGETS = {
        'vercel' => %w[turso neon planetscale supabase dexie],
        'cloudflare' => %w[d1 turso dexie],
        'deno-deploy' => %w[turso neon planetscale supabase],
        'fly' => %w[mpg pg postgres postgresql]
      }.freeze
      ...
      class << self
        ...
        def infer_target_from_database(database)
          ...
          # Only return deploy-able targets (vercel, cloudflare, deno-deploy, fly)
          target if %w[vercel cloudflare deno-deploy fly].include?(target)
        end
        ...
        def generate_platform_config(target, options)
          ...
          when 'deno-deploy'
            generate_deno_deploy_config(options)
          when 'fly'
            generate_fly_config(options)
          end
        end
        ...
        def generate_deno_deploy_config(options)
          # Deno Deploy uses deployctl which reads from main.ts
          # The entry point is already generated by the builder
          # Generate a deno.json for import map if needed
          app_name = File.basename(Dir.pwd).downcase.gsub(/[^a-z0-9-]/, '-')
          ...
          deno_json = {
            ...
            "imports" => {
              # Add any import mappings if needed
            }
          }

          File.write(File.join(DIST_DIR, 'deno.json'), JSON.pretty_generate(deno_json))
          puts "  Generated deno.json"
        end

        def generate_fly_config(options)
          # Fly.io config is already generated by the builder (fly.toml, Dockerfile)
          # Just verify the files exist
          fly_toml = File.join(DIST_DIR, 'fly.toml')
          dockerfile = File.join(DIST_DIR, 'Dockerfile')

          unless File.exist?(fly_toml)
            abort "Error: fly.toml not found in dist/.\n" \
                  "Run 'juntos build -t fly -d mpg' first."
          end

          unless File.exist?(dockerfile)
            abort "Error: Dockerfile not found in dist/.\n" \
                  "Run 'juntos build -t fly -d mpg' first."
          end

          puts "  fly.toml and Dockerfile ready"
        end
        ...
        def verify_deployment(target, options)
          ...
          Dir.chdir(DIST_DIR) do
            ...
            when 'deno-deploy'
              "./main.ts"
            when 'fly'
              "./config/routes.js"  # Fly uses Node.js server
            else
              "./config/routes.js"
            end
        ...
        def run_platform_deploy(target, options)
          ...
          when 'deno-deploy'
            Dir.chdir(DIST_DIR) do
              project_name = File.basename(Dir.pwd).downcase.gsub(/[^a-z0-9-]/, '-')
              if system("which deployctl > /dev/null 2>&1")
                exec("deployctl", "deploy", "--project=#{project_name}", "main.ts")
              else
                puts "\nTo deploy, install deployctl and run:"
                puts "  deno install -Arf jsr:@deno/deployctl"
                puts "  cd dist && deployctl deploy --project=#{project_name} main.ts"
              end
            end
          when 'fly'
            Dir.chdir(DIST_DIR) do
              if system("which fly > /dev/null 2>&1")
                exec("fly", "deploy")
              else
                puts "\nTo deploy, install the Fly CLI and run:"
                puts "  curl -L https://fly.io/install.sh | sh"
                puts "  cd dist && fly deploy"
              end
            end
          end
        end
