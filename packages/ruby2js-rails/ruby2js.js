...
const Ruby2JS = (() => {
  ...
  class Converter extends Serializer {
    ...
    timestamp(file) {
      this._parent.timestamp.call(this, file);
      if (!file) return;
    ...
    on_class2(name, inheritance, ...body) {
...

      {
        ...
        try {
          let ivars, cvars, walk, references_args, rename, forward, proxy;
          [class_name, this._class_name] = [this._class_name, name];
          [class_parent, this._class_parent] = [this._class_parent, inheritance];
    ...
    on_module(name, ...body) {
      ...
      body.forEach((node, i) => {
        ...
        if (node.type === "casgn" && node.children.first == null) {
          ...
          if (node_comments && (Array.isArray(node_comments) ? node_comments.some(Boolean) : node_comments)) {
    ...
    on_send(receiver, method, ...args) {
      ...
      if (["await", "await!"].includes(this._ast.type)) this.put("await ");
      ...
      if (method === "!") {
        ...
        if (!this._ast.is_method() && !["send!", "call", "await!"].includes(this._ast.type)) {
          if (receiver) {
            if (group_receiver) {
              this.group(receiver)
            } else {
    ...
    output_pnode_child_inline(child) {
      ...
      case "pnode":
        let [tag, attrs, ...grandchildren] = child.children;
    ...
    output_pnode_child_with_loops(child) {
      let content, tag, attrs, grandchildren;
      ...
      case "pnode_text":
        ...
        if (content.type === "str") {
          let text = content.children.first;
  ...
  class Pipeline {
    ...
    get reassociate_comments() {
      ...
      indices = indices.slice().sort((i_a, i_b) => {
        if ((node_starts[i_a] || 0) < (node_starts[i_b] || 0)) {
          return -1
        } else if ((node_starts[i_a] || 0) > (node_starts[i_b] || 0)) {
          return 1
        } else {
          return 0
        }
  ...
  const JSX_ALL_ELEMENTS = Object.freeze(JSX_VOID_ELEMENTS + JSX_HTML_ELEMENTS);
        ...
        try {
          ...
          while (true) {
            ...
            case "close":
              ...
              if (c === ">") {
                let tag_info = this._tag_stack.last;
...
export function parse(source, file=null) {
  ...
  return [ast, walker.source_buffer]
};
