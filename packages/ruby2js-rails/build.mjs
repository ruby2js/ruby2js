...
export class SelfhostBuilder {
  ...
  static generate_package_json(options={}) {
    ...
    let local_package = path.join(gem_root, "packages/ruby2js-rails");
    ...
    // React - only needed if app has .jsx.rb views
    // Apps can override versions in config/ruby2js.yml
    let has_jsx_views = false;
    for (const dir of ['app/views', 'app/components']) {
      let check_dir = app_root ? path.join(app_root, dir) : dir;
      if (fs.existsSync(check_dir)) {
        try {
          let found = fs.readdirSync(check_dir, { recursive: true })
            .some(f => f.endsWith('.jsx.rb'));
          if (found) { has_jsx_views = true; break; }
        } catch (e) {}
      }
    }
    if (has_jsx_views) {
...
`
  ...
  copy_database_adapter() {
    ...
    let npm_dist_dir = path.join(
      this._dist_dir,
      "node_modules/ruby2js-rails/dist/lib"
    );
    ...
    let adapter_dir = fs.existsSync(pkg_adapter_dir) ? pkg_adapter_dir : fs.existsSync(npm_adapter_dir) ? npm_adapter_dir : fs.existsSync(npm_dist_dir) ? npm_dist_dir : fs.existsSync(vendor_adapter_dir) ? vendor_adapter_dir : (() => { throw `Could not find ruby2js-rails adapters directory.
Looked in:
  - ${npm_adapter_dir}
  - ${npm_dist_dir}
  - ${pkg_adapter_dir}
...
`;
  ...
  process_stimulus_controllers(src_dir, dest_dir) {
    ...
    if (controllers.some(Boolean)) {
      return this.generate_stimulus_index(dest_dir, controllers.uniq)
    }
  };
