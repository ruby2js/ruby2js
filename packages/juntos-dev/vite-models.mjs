...
import { modelRegistry } from 'juntos/adapters/active_record_dexie.mjs';
${imports}
...
`;
  ...
  async function generateBridgeFiles(modelsDestDir, migrationsDestDir) {
    ...
    const adapterPath = `juntos/adapters/${adapterFile}`;
...
export { BroadcastChannel } from 'juntos/targets/${railsTarget}/rails.js';
`;
      await fs.promises.writeFile(
        path.join(modelsLibDir, 'rails.js'),
        railsBridge
