...
describe Ruby2JS::Filter::Functions do
  ...
  describe 'fs' do
    it 'should handle IO.read' do
      to_js( 'IO.read("foo")' ).
        must_equal 'const fs = require("fs"); fs.readFileSync("foo", "utf8")'
    end
    ...
    it 'should handle File.readlink' do
      to_js( 'File.readlink("foo")' ).
        must_equal 'const fs = require("fs"); fs.readlinkSync("foo")'
    end

    it 'should handle File.realpath' do
      to_js( 'File.realpath("foo")' ).
        must_equal 'const fs = require("fs"); fs.realpathSync("foo")'
    end

    it 'should handle File.rename' do
      to_js( 'FileUtils.mv("foo", "bar")' ).
        must_equal 'const fs = require("fs"); fs.renameSync("foo", "bar")'
    ...
    it 'should handle File.symlink' do
      to_js( 'File.symlink("foo", "bar")' ).
        must_equal 'const fs = require("fs"); fs.symlinkSync("foo", "bar")'
    ...
    it 'should handle File.truncate' do
      to_js( 'File.truncate("foo", 0)' ).
        must_equal 'const fs = require("fs"); fs.truncateSync("foo", 0)'
    end

    it 'should handle File.stat' do
      to_js( 'File.stat("foo")' ).
        must_equal 'const fs = require("fs"); fs.statSync("foo")'
    end

    it 'should handle File.lstat' do
      to_js( 'File.lstat("foo")' ).
        must_equal 'const fs = require("fs"); fs.lstatSync("foo")'
    end

    it 'should handle File.unlink' do
      to_js( 'File.unlink("foo")' ).
        must_equal 'const fs = require("fs"); fs.unlinkSync("foo")'
    ...
    it 'should handle FileUtils.cp' do
      to_js( 'FileUtils.cp("foo", "bar")' ).
        must_equal 'const fs = require("fs"); fs.copyFileSync("foo", "bar")'
    ...
    it 'should handle Dir.entries' do
      to_js( 'Dir.entries("foo")' ).
        must_equal 'const fs = require("fs"); fs.readdirSync("foo")'
    end

    it 'should handle Dir.mktmpdir' do
      to_js( 'Dir.mktmpdir' ).
        must_equal 'const fs = require("fs"); fs.mkdtempSync("d")'
