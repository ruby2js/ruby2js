...
// Application base class for Deno
...
// Navigate helper - for Deno, this just returns the path
...
// Form submission helper - for Deno, forms are handled via HTTP
...
// Extract form data - for Deno, data comes from request body
export function formData(event) {
  // In server context, form data is parsed in Router.parseBody
  return {};
}

// Handle form result - for Deno, this is handled in Router.handleResult
export function handleFormResult(result, rerenderFn = null) {
  return false;
}

// Setup form handlers - no-op in Deno (forms handled via HTTP)
export function setupFormHandlers(config) {
  // Form handlers are handled by Router.dispatch in Deno
}
