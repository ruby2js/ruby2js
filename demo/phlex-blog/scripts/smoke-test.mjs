#!/usr/bin/env node
// Smoke test for Phlex Blog demo builds
...
async function runTests() {
  log('\n=== Phlex Blog Smoke Test ===\n', BOLD);

  let passed = 0;
  let failed = 0;

  const tempDir = mkdtempSync(join(tmpdir(), 'phlex-blog-test-'));
  ...
  try {
    ...
    try {
      ...
      mkdirSync(join(rubyDist, 'views'), { recursive: true });
      execSync(`bundle exec ruby demo/phlex-blog/scripts/build.rb`, {
        cwd: PROJECT_ROOT,
        encoding: 'utf8',
        stdio: ['pipe', 'pipe', 'pipe']
      });
      // Copy component files
      const componentsDir = join(DEMO_ROOT, 'dist/components');
      ...
      // Copy view files
      const viewsDir = join(DEMO_ROOT, 'dist/views');
      if (existsSync(viewsDir)) {
        for (const f of readdirSync(viewsDir)) {
          if (f.endsWith('.js')) {
            const content = readFileSync(join(viewsDir, f), 'utf8');
            writeFileSync(join(rubyDist, 'views', f), content);
          }
        }
      }
    ...
    try {
      ...
      mkdirSync(join(selfhostDist, 'views'), { recursive: true });
      await buildWithSelfhost(
        join(DEMO_ROOT, 'app/components'),
        join(selfhostDist, 'components')
      );
      await buildWithSelfhost(
        join(DEMO_ROOT, 'app/views'),
        join(selfhostDist, 'views')
      );
