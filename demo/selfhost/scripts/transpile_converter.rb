...
# Extract polyfills from the beginning (before const/let Ruby2JS =)
# They are generated by the polyfill filter and need to be in the preamble
polyfills = ""
if js =~ /\A((?:(?:if\s*\(!|Object\.defineProperty).*?;?\n)+)((const|let)\s+Ruby2JS\s*=)/m
  polyfills = $1
  js = js[$1.length..]
end

# Remove const/let prefix from Ruby2JS (we'll add export const in preamble)
js = js.sub(/^(const|let)\s+(Ruby2JS\s*=)/, '\2')
