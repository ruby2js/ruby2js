#!/usr/bin/env ruby
# Transpile SvelteComponentTransformer from Ruby to JavaScript for selfhost use
...
source_file = File.expand_path('../../../lib/ruby2js/svelte_component_transformer.rb', __dir__)
...
source = source.sub(
  /Result = Struct\.new\(:component, :script, :template, :imports, :errors, keyword_init: true\)/,
  <<~RUBY.strip
    def self.Result(component: nil, script: nil, template: nil, imports: nil, errors: nil)
      {component: component, script: script, template: template, imports: imports, errors: errors}
    end
  RUBY
)
...
# Remove require for svelte_template_compiler - we'll import it instead
source = source.gsub(/require ['"]ruby2js\/svelte_template_compiler['"]/, '')
...
# Fix Result() calls - need class prefix in JS (SvelteComponentTransformer.Result)
# But don't modify the static method definition itself
js = js.gsub(/return Result\(/, 'return SvelteComponentTransformer.Result(')
...
js = js.gsub(/for \(let \[(\w+), (\w+)\] of (SvelteComponentTransformer\.LIFECYCLE_HOOKS)\)/) do
