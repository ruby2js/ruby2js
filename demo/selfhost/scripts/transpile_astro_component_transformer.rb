#!/usr/bin/env ruby
# Transpile AstroComponentTransformer from Ruby to JavaScript for selfhost use
...
source_file = File.expand_path('../../../lib/ruby2js/astro_component_transformer.rb', __dir__)
...
# Remove require for astro_template_compiler - we'll import it instead
source = source.gsub(/require ['"]ruby2js\/astro_template_compiler['"]/, '')
...
# Fix Result() calls - need class prefix in JS (AstroComponentTransformer.Result)
# But don't modify the static method definition itself
js = js.gsub(/return Result\(/, 'return AstroComponentTransformer.Result(')
