...
describe("full pipeline - method mappings", () => {
  it("should convert length/size", () => {
    convert("arr = [1,2]; arr.length").must_include(".length");
    convert("arr = [1,2]; arr.size").must_include(".length");
  });

  it("should convert array methods", () => {
    convert("arr = [1]; arr.push(2)").must_include(".push(2)");
    convert("arr = [1]; arr.first").must_include("[0]");
    convert("arr = [1,2]; arr.include?(1)").must_include(".includes(1)");
    convert("arr = [1,2]; arr.join(',')").must_include(".join");
  });

  it("should convert string methods", () => {
    convert("s = 'hi'; s.upcase").must_include(".toUpperCase()");
    convert("s = 'hi'; s.downcase").must_include(".toLowerCase()");
    convert("s = '  hi  '; s.strip").must_include(".trim()");
  });

  it("should convert type conversions", () => {
    convert("x = '5'; x.to_i").must_include("parseInt");
    convert("x = 5; x.to_s").must_include(".toString()");
  });

  it("should convert hash methods", () => {
    convert("h = {a: 1}; h.keys").must_include("Object.keys");
    convert("h = {a: 1}; h.values").must_include("Object.values");
  });

  it("should convert numeric methods", () => {
    convert("x = -5; x.abs").must_include("Math.abs");
    convert("x = 3.7; x.round").must_include("Math.round");
  });

  it("should convert Foo.new to new Foo", () => {
    convert("Foo.new(1, 2)").must_include("new Foo(1, 2)");
  });
});

describe("full pipeline - exception handling", () => {
  it("should convert begin/rescue", () => {
    const result = convert("begin; x; rescue => e; puts e; end");
    result.must_include("try {");
    result.must_include("catch (e)");
  });

  it("should convert begin/ensure", () => {
    const result = convert("begin; x; ensure; cleanup; end");
    result.must_include("try {");
    result.must_include("finally {");
  });

  it("should convert raise to throw", () => {
    convert("raise 'error'").must_include("throw new Error");
  });
});
