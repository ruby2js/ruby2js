...
//   (...))
//
// Synthetic node type for logical OR operator (||)
// Forces || operator even when @or option would normally use ??
// (op-asgn
//   (lvasgn :a) :+
//   (int 1))
...
// For endless method detection
// nil means no 'end' keyword (endless)
// Create expression for sourcemap generation
...
// Open block scope to contain hoisted vars
...
// Note: This class is designed to be transpilable to JavaScript. It uses
// JSON-serialized strings as hash keys (instead of arrays) for JS compatibility.
//
// current scope
// history of all definitions seen previously
...
//   (...))
//
// Synthetic node type for logical OR assignment operator (||=)
// Forces ||= operator even when @or option would normally use ??=
// Note: use explicit element comparison for JS compatibility (array == array compares refs in JS)
...
export function convert(source, options={}) {
  ...
  // Copy array so we can modify defs nodes (works in Ruby and JS)
