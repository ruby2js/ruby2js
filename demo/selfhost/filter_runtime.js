...
// Basic Inflector for Rails filters
...
Ruby2JS.Inflector = {
  ...
  _uncountables: new Set([
    'equipment', 'information', 'rice', 'money', 'species', 'series',
    'fish', 'sheep', 'deer', 'aircraft', 'news', 'feedback'
  ]),
  ...
  pluralize(word) {
    ...
    // Check uncountables
    if (this._uncountables.has(lower)) return word;

    // Check irregulars
    if (this._irregulars[lower]) {
      return word[0] === word[0].toUpperCase()
        ? this._irregulars[lower].charAt(0).toUpperCase() + this._irregulars[lower].slice(1)
        : this._irregulars[lower];
    }
  ...
  singularize(word) {
    ...
    for (const [singular, plural] of Object.entries(this._irregulars)) {
      if (lower === plural) {
        return word[0] === word[0].toUpperCase()
