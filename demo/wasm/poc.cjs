// Node.js proof of concept for Ruby2JS + ruby.wasm (CommonJS)
const fs = require('fs');
const { WASI } = require('wasi');
const { RubyVM } = require('@ruby/wasm-wasi');

const wasmFile = process.argv[2] || './ruby2js-3.4.wasm';
...
async function main() {
  try {
    ...
    console.log(`  File read: ${((performance.now() - startTime) / 1000).toFixed(2)}s`);
